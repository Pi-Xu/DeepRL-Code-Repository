...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            for i in range(self.cem_iterations):
                ...
                # make sure that the acs in action space
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        obs_batch = np.asarray([obs]*N)
        sum_of_rewards = np.zeros(N)
        for t in range(H):

            action = candidate_action_sequences[:,t,:]
            reward, done = self.env.get_reward(obs_batch, action)
            obs_batch = model.get_prediction(obs_batch, action, self.data_statistics)
