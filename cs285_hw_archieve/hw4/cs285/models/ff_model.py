...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = ptu.to_numpy(
            self(obs, acs, self.obs_mean, self.obs_std, self.acs_mean, self.acs_std, self.delta_mean, self.delta_std)[0]
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target = normalize(next_observations - observations, self.delta_mean, self.delta_std) # TODO(Q1) compute the normalized target for the model.
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.

        prediction = self(observations, actions, self.obs_mean, self.obs_std, self.acs_mean, self.acs_std, self.delta_mean, self.delta_std)[1]
        loss = self.loss(
            target, prediction
