...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            values = utils.unnormalize(utils.normalize(values_unnormalized, np.mean(values_unnormalized), np.std(values_unnormalized)), np.mean(q_values), np.std(q_values))
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i] == 1: # according to hint 1
