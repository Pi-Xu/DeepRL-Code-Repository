...
class FlexibleReplayBuffer(object):
    def __init__(self, size, horizon) -> None:
        self.size = size
        self.next_idx = 0
    ...
    def sample_recent_data(self, batch_size, concat_rew=True):
        assert self.can_sample(batch_size)
        data_range = range(self.next_idx-batch_size-1 ,self.next_idx-1)
        if concat_rew:
            return self._encode_sample(idxes=[idx % self.size for idx in data_range])
        else:
            cur, i, unconcat_rew = (self.next_idx - 1) % self.size, 0, []
            while i < batch_size:
                if not self.done[cur]:
                    ...
                    i += 1
                    cur = (cur - 1) % self.size
                while i < batch_size and not self.done[cur]:
